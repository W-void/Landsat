...
def train(epo_num=10):
    ...
    net_pretrained = None
    net_pretrained = torch.load("./checkpoints_unet/unet_2.pt")
    # net_pretrained = torch.load("./checkpoints_attention/SegNet_2.pt")
    # net_pretrained = torch.load("./checkpoints_attention/SpoonNetSpretral3_12.pt")
    ...
    for epo in range(epo_num):
        ...
        with torch.no_grad():
            for index, (names, bag, bag_msk, _) in enumerate(test_dataloader):
                ...
                acc_test, recall_test, precision_test = get_acc_recall_precision(evaluateArray, bag_msk.data.cpu(), outputData)
