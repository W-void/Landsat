...
def test(modelPath):
    ...
    # unet = torch.load("./checkpoints_unet/unet_2.pt")
    # total_params = sum(p.numel() for p in unet.parameters())
    # print(total_params)
    # unet = unet.to(device).float()
    # unet.eval()

    segnet = torch.load("./checkpoints_attention/SegNet_9.pt")
    total_params = sum(p.numel() for p in segnet.parameters())
    print(total_params)
    segnet = segnet.to(device).float()
    segnet.eval()

    # spoonnet = torch.load("./checkpoints_attention/SpoonNet_5.pt")
    # spoonnet = torch.load("./checkpoints_attention/SpoonNetSpretral3_12.pt")
    ...
    for epo in range(1):
        for index, (names, bag, bag_msk, qa) in enumerate(test_dataloader):
            ...
            # spectral = spectral[:, 0:3].detach().cpu().numpy()
            # spatial = spatial[:, 0:3].detach().cpu().numpy()
            # m_outputData = torch.argmax(m_output.data, 1)
            # u_outputData = torch.argmax(u_output.data, 1)
            s_outputData = torch.argmax(s_output.data, 1)
            ...
            for i, name in enumerate(names):
                print(name, senceList[senceDict[name.split('_')[0]]])
                # s = spectral[i]
                # for j in range(3):
                #     s[j] = (s[j] -  s[j].min())/  (s[j].max() - s[j].min())
                # q = qa[i].float().cpu().numpy()
                # u_out = u_outputData[i].float().cpu().numpy()
                # m_out = m_outputData[i].float().cpu().numpy()
                mask = bag_msk[i].float().cpu().numpy()
                color = np.transpose(bag[i, 1:4].cpu().numpy() * 6e-6, (1, 2, 0))
                s_out = s_outputData[i].float().cpu().numpy()
                ...
                if k == ord('s'):
                    ...
                    cv2.imwrite(savePath+imgName+'_segnet.jpg', s_out*255)
