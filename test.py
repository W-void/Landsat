...
def test(modelPath):
    ...
    # net = torch.load("./checkpoints_attention/SpoonNet3_6.pt", map_location=torch.device('cpu'))
    ...
    net2 = torch.load("./checkpoints_attention/unet_4.pt")
    ...
    # net3 = torch.load("./checkpoints_attention/SegNet_9.pt")
    ...
    for epo in range(1):
        ...
        for index, (names, bag, bag_msk, qa) in enumerate(test_dataloader):
            ...
            with torch.no_grad():
                ...
                # outputData = np.argmax(output.data.cpu(), 1)

                output2 = net2(bag)
                outputData2 = np.argmax(output2.data.cpu(), 1)

                # output3 = net3(bag)
                # outputData3 = np.argmax(output3.data.cpu(), 1)
...
def regionSelect(datas):
    for i, data in enumerate(datas):
        labels = measure.label(data.cpu(), connectivity=1)  #8连通区域标记
        #筛选连通区域大于30的
